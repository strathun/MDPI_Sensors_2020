...
%% Plot Percent changes for each condition

for ii = 1:4
    % 1xPBS
    ZdeltaStructure(1).Zdelta(ii, :) = customStructure_1(ii + 2).Zmag(:,5) - ...
        customStructure_1(2).Zmag(:,5);
    ZdeltaStructure(1).Zpercent_delta(ii, :) =  (ZdeltaStructure(1).Zdelta(ii, :)./ ...
        customStructure_1(2).Zmag(:,5).')*100;
    % 0.1xPBS
    ZdeltaStructure(2).Zdelta(ii, :) = customStructure_0p1(ii + 1).Zmag(:,5) - ...
        customStructure_0p1(1).Zmag(:,5);
    ZdeltaStructure(2).Zpercent_delta(ii, :) =  (ZdeltaStructure(2).Zdelta(ii, :)./ ...
        customStructure_0p1(1).Zmag(:,5).')*100;
    % 2xPBS
    ZdeltaStructure(3).Zdelta(ii, :) = customStructure_2(ii + 1).Zmag(:,5) - ...
        customStructure_2(1).Zmag(:,5);
    ZdeltaStructure(3).Zpercent_delta(ii, :) =  (ZdeltaStructure(3).Zdelta(ii, :)./ ...
        customStructure_2(1).Zmag(:,5).')*100;
end

figure
colorArray = lines( 4 );
for ii = 1:4
    semilogx( customStructure_0p1(1).f(:, 5).', ...
              ZdeltaStructure(2).Zpercent_delta(ii, :), ...
               'Color', colorArray( ii, : ), ...
               'LineWidth', 1.5)
    hold on
    semilogx( customStructure_1(2).f(:, 5).', ...
              ZdeltaStructure(1).Zpercent_delta(ii, :), ...
               'o', 'Color', colorArray( ii, : ), ...
               'LineWidth', 1.5)
    semilogx( customStructure_2(1).f(:, 5).', ...
              ZdeltaStructure(3).Zpercent_delta(ii, :), ...
               '*', 'Color', colorArray( ii, : ), ...
               'LineWidth', 1.5)
end
ylabel('% Impedance Change')
xlabel('Frequency (Hz)')
legend('0p1x PBS', '1x PBS', '2x PBS')
