%% data from 20201130
% Just a quick test to verify I remember what I'm doing with the custom
% pstat
...
[gamryStructure] = ...
    extractImpedanceDataGlobal('..\rawData\Gamry\20201130_TDT22_InVitro_1xPBS');

[customStructure] = ...
    extractCustomPstatData('..\rawData\CustomPot\20201130_TDT22_InVitro_1xPBS');
...
for ii = 1:1
    loglog( gamryStructure(kk).f, ...
            ...
            'Color', colorArray( ii, : ))
        hold on
    loglog( gamryStructure(kk+1).f, ...
            gamryStructure(kk+1).Zmag, ...
            'Color', colorArray( ii, : ))
    % Convert from Tye's pinout to gamry
%     [ jj ] = pinoutConverter( 'gamry', 'customPot', 'trodeSpecifier', ii );
    loglog( customStructure(1).f(:,16), ...
            customStructure(1).Zmag(:,16) , 'o', ...
            'Color', colorArray( ii, : ))
    kk = kk + 2;
    ll = ll + 2;
end
